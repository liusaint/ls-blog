### 我的github博客 日常总结记录
更多早期博客在CSDN上。http://blog.csdn.net/liusaint1992

部分文章效果演示地址：http://runningls.com/demos/

文章基本是写在项目的issues中。欢迎star，一起学习！

#### 2020.5
* [微信小程序填坑 视频篇](https://github.com/liusaint/ls-blog/issues/40)

#### 2020.4
* [英语字母笔画绘制功能解析](https://github.com/liusaint/ls-blog/issues/39)
* [学习类产品100+资源加载体验优化 ](https://github.com/liusaint/ls-blog/issues/38)


#### 2019.10
* [动物餐厅 小游戏 自助添加物品](https://github.com/liusaint/ls-blog/issues/37)

#### 2019.04
* [aeneas实现音频强制对齐](https://github.com/liusaint/ls-blog/issues/36)

#### 2019.01
* [前后端分离项目中使用富文本编辑器UEditor](https://github.com/liusaint/ls-blog/issues/34)

#### 2018.11
* [React使用小结](https://github.com/liusaint/ls-blog/issues/33)
* [ant-design-pro 使用总结—自定义打包构建配置](https://github.com/liusaint/ls-blog/issues/32)


#### 2018.8
* [微信 jssdk本地调试方案](https://github.com/liusaint/ls-blog/issues/30)

#### 2018.6
* [使用chrome浏览器插件抢小米8](https://github.com/liusaint/ls-blog/blob/master/code/mi/readme.md)
* [常见的web攻击方式及预防](https://github.com/liusaint/ls-blog/issues/29)

#### 2018.5
* [前端基本功之从大型项目中迅速定位修改位置](https://github.com/liusaint/ls-blog/issues/27)
* [大型canvas 2d应用的事件处理](https://github.com/liusaint/ls-blog/issues/26)
* [二叉树遍历（前序，后序，中序，层次）递归与迭代实现JavaScript](https://github.com/liusaint/ls-blog/issues/25)

#### 2018.3
* [前端性能优化小结](https://github.com/liusaint/ls-blog/issues/22)

#### 2018.2
* [探究点击事件在JavaScript事件循环中的表现](https://github.com/liusaint/ls-blog/issues/20)

#### 2018.1
* [异步编程的一个场景](https://github.com/liusaint/ls-blog/issues/14)
* [再说IE9中的console](https://github.com/liusaint/ls-blog/issues/15)
* [JavaScript面向对象](https://github.com/liusaint/ls-blog/issues/16)
* [知乎快捷取消我关注的问题chrome插件](https://github.com/liusaint/ls-blog/issues/17)
* [前端填坑指南（提纲）](https://github.com/liusaint/ls-blog/issues/18)

#### 2017.12
* [记录一个bug，逻辑错误，结果正确，异步代码调试及IE下的渲染机制](https://github.com/liusaint/ls-blog/issues/13)

#### 2017.11
* [jQuery Placeholder Plugin源码分析](https://github.com/liusaint/ls-blog/issues/11)

#### 2017.10
* [模拟multiple select，实现不按ctrl单击选中以及拖动选择](https://github.com/liusaint/JavaScript-record/issues/9)
* [复杂单页系统维护与优化经验(待续)](https://github.com/liusaint/ls-blog/issues/10)
* [在项目中自定义路径放入element-ui并修改编译源码](http://blog.csdn.net/liusaint1992/article/details/78238096)

#### 2017.9
* [Object.defineProperty()的set方法除了用于双向绑定还能干嘛？](https://github.com/liusaint/JavaScript-record/issues/6)
* [IE踩坑记](https://github.com/liusaint/JavaScript-record/issues/7)
* [个人总结的前端开发规范](https://github.com/liusaint/JavaScript-record/issues/8)

#### 2017.8
* [一个不打开浏览器工具的调试方法](https://github.com/liusaint/JavaScript-record/issues/4)
* [在iframe中优雅地使用父窗口中的$](https://github.com/liusaint/JavaScript-record/issues/5)

#### 2017.6
* [几种跨域处理方式](https://github.com/liusaint/JavaScript-record/issues/1)
* [nodejs读取本地文件夹的文件备份文件名](https://github.com/liusaint/JavaScript-record/issues/2)
* [JavaScript中正则表达式常见使用函数](https://github.com/liusaint/JavaScript-record/issues/3)

#### 更早（2017年以前）
* [JavaScript九宫格数独生成算法](http://blog.csdn.net/liusaint1992/article/details/51147149)
* [JavaScript排序算法动画演示效果实现](http://blog.csdn.net/liusaint1992/article/details/51656124)
* [HTML5实现经典Windows扫雷游戏](http://blog.csdn.net/liusaint1992/article/details/50531186)
* [HTML5小游戏贪吃蛇分析与实现](http://blog.csdn.net/liusaint1992/article/details/50513181)
* [HTML/CSS/JavaScript实现的2048小游戏](http://blog.csdn.net/liusaint1992/article/details/51549596)
* [根据经纬度绘制坐标点相对位置算法（分别用php和html5实现）](http://blog.csdn.net/liusaint1992/article/details/49668571)
* [根据卫星仰角和方位角使用html5绘制天空图（投影图）](http://blog.csdn.net/liusaint1992/article/details/49835933)
* [自己封装的JSONP跨域函数](http://blog.csdn.net/liusaint1992/article/details/50959571)
* [获取onbeforeunload的返回结果，在检测到取消离开页面后执行操作](http://blog.csdn.net/liusaint1992/article/details/49648165)
* [纯css，div隐藏滚动条，保留鼠标滚动效果](http://blog.csdn.net/liusaint1992/article/details/51277751)
* [天地图API使用系列　7篇](http://blog.csdn.net/liusaint1992/article/category/5854891)
* [百度地图API使用系列　5篇](http://blog.csdn.net/liusaint1992/article/category/5985931)

